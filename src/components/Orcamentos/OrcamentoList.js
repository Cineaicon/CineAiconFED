...
const OrcamentoList = () => {
  ...
  const handleGeneratePDF = async (tipo) => {
    try {
      ...
      link.href = url;
      const jobNamePart = selectedOrcamento.jobName ? `-${selectedOrcamento.jobName.replace(/[\\/:*?"<>|]/g, '').trim()}` : '';
      link.setAttribute('download', `${tipo}-${selectedOrcamento.numero}${jobNamePart}.pdf`);
